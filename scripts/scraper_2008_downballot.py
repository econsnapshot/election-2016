...
file_nat = open(data_senate_path + "national_2008.csv", "w")
...
for state in states:
    try:
        i = 1
        file_nat = open(data_senate_path + "national_2008.csv", "a")
        file_state = open(data_senate_path + state + "_2008.csv", "w")
        ...
        while i < 100:
            ...
            url = website_president + state.upper() + 'S00p' + str(i)
            driver.get(url)
            time.sleep(5)
...
file_nat = open(data_house_path + "national_2008.csv", "w")
file_nat.write("State,District,Party,Candidate,Percentage,Votes\n")
file_nat.close()

for state in states:
    file_state = open(data_house_path + state + "_2008.csv", "w")
...
while i < 100:
    file_nat = open(data_house_path + "national_2008.csv", "a")
    driver = webdriver.PhantomJS(executable_path = phantom_exe)
    url = website_house + str(i)
    driver.get(url)
    time.sleep(5)
    soup = BeautifulSoup(driver.page_source)
    for res_table in soup.find_all('div', class_='cnnElex_rBox'):
        county_name = res_table.find('div', class_='cnnElex_raceState').find('span').get_text()
        try:
            state = state_mapping[str(county_name.split(" ")[0])]
        except:
            state = state_mapping[str(county_name.split(" ")[0]) + " " + str(county_name.split(" ")[1])]
        file_state = open(data_house_path + state + "_2008.csv", "a")
        for tr in res_table.find_all('tr'):
            tds_name = tr.find('a', class_='nobio').get_text()
            if tr.find('div', class_='cnnR_R'):
                tds_party = "R"
                ...
                tds_party = 'Unknown'
            if tr.find('td', class_='unopposed'):
                tds_pct = 'unopposed'
                tds_tot = 'unopposed'
            else:
                tds_pct = tr.find('b').get_text()
...
file_nat = open(data_governor_path + "national_2008.csv", "w")
file_nat.write("State,County,Party,St_FIPS,County_FIPS,Candidate,Percentage,Votes\n")
file_nat.close()
...
for state in states:
    try:
        i = 1
        file_nat = open(data_governor_path + "national_2008.csv", "a")
        file_state = open(data_governor_path + state + "_2008.csv", "w")
        file_state.write("County,County_FIPS,Party,Candidate,Percentage,Votes\n")
        while i < 100:
            driver = webdriver.PhantomJS(executable_path = phantom_exe)
            url = website_president + state.upper() + 'G00p' + str(i)
            driver.get(url)
            time.sleep(5)
            soup = BeautifulSoup(driver.page_source)
            for res_table in soup.find_all('div', class_='cnnElex_rBoxCTY'):
